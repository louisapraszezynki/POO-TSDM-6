...
# Actions
    ...
    def attack(self, skill, target):
        ...
        # Coup critique ?
        chiffre_crit = random.randint(0, 100)

        if chiffre_crit < self.crit:
            pass
        else:
            target.health -= 2 * (total_attack)
...
class Mage(Unit):
    ...
    actions = ['Spell', 'Regen']
    skills = [
        Spell('spell', 4, 10, 3), # attack_type:str, range:int, power:int, area_of_effect:int
