...
class Game:
    ...
    def display_actions(self):
        """
        Affiche les deux actions possibles pour le personnage sélectionné.
        """
        if self.selected_character:
            print(f"Actions possibles pour {self.selected_character.name}:")
            for i, action in enumerate(self.selected_character.actions, 1):
                print(f"{i}. {action['name']} - {action['description']}")
        else:
            print("Aucun personnage sélectionné.")

    def select_action(self, action_number):
        """
        Permet au joueur de sélectionner une action avec les touches chiffres.
        """
        if not self.selected_character:
            print("Aucun personnage sélectionné.")
            return

        if 1 <= action_number <= len(self.selected_character.actions):
            self.current_action = self.selected_character.actions[action_number - 1]
            print(f"Vous avez sélectionné l'action : {self.current_action['name']}")
            self.turn_over = True  # Fin du tour après avoir choisi une action
        else:
            print("Sélection invalide. Veuillez choisir une action valide.")

    def display_action_range(self):
        """
        Affiche la portée de l'action sélectionnée.
        """
        if self.current_action:
            range_area = self.current_action.get('range', 0)
            print(f"Portée de l'action {self.current_action['name']}: {range_area}")
        else:
